...
/// Reads a property that is simply on or off.
fn find_flag(text_node: SvgNode, aid: AId) -> Option<bool> {
    match text_node.find_attribute::<&str>(aid)? {
        "true" => Some(true),
        "false" => Some(false),
        value => {
...
/// Resolves the hinting of a text element.
///
/// The document may only pick between the ways of hinting the host allowed, so
/// the `-resvg-hinting-*` properties adjust what `Options::hinting` configured
/// and cannot switch hinting on by themselves.
fn resolve_hinting(text_node: SvgNode, state: &converter::State) -> Option<crate::HintingOptions> {
    let mut options = state.opt.hinting?;

    if text_node.find_attribute::<crate::TextHinting>(AId::ResvgHinting)
        == Some(crate::TextHinting::None)
    {
        return None;
    }

    if let Some(engine) = text_node.find_attribute(AId::ResvgHintingEngine) {
        options.engine = engine;
    }

    // The settings below only exist on a smooth target, so they are carried
    // over from the configured one, or from the defaults when a mono target was
    // configured and the document asks for a smooth one.
    let (mut mode, mut symmetric, mut preserve) = match options.target {
        crate::HintingTarget::Smooth {
            mode,
            symmetric_rendering,
            preserve_linear_metrics,
        } => (mode, symmetric_rendering, preserve_linear_metrics),
        crate::HintingTarget::Mono => match crate::HintingTarget::default() {
            crate::HintingTarget::Smooth {
                mode,
                symmetric_rendering,
                preserve_linear_metrics,
            } => (mode, symmetric_rendering, preserve_linear_metrics),
            crate::HintingTarget::Mono => unreachable!("the default target is smooth"),
        },
    };

    if let Some(value) = text_node.find_attribute(AId::ResvgHintingMode) {
        mode = value;
    }
    if let Some(value) = find_flag(text_node, AId::ResvgHintingSymmetric) {
        symmetric = value;
    }
    if let Some(value) = find_flag(text_node, AId::ResvgHintingPreserveLinearMetrics) {
        preserve = value;
    }

    let kind = text_node
        .find_attribute(AId::ResvgHintingTarget)
        .unwrap_or(match options.target {
            crate::HintingTarget::Mono => crate::HintingTargetKind::Mono,
            crate::HintingTarget::Smooth { .. } => crate::HintingTargetKind::Smooth,
        });

    options.target = match kind {
        crate::HintingTargetKind::Mono => crate::HintingTarget::Mono,
        crate::HintingTargetKind::Smooth => crate::HintingTarget::Smooth {
            mode,
            symmetric_rendering: symmetric,
            preserve_linear_metrics: preserve,
        },
    };
...
pub(crate) fn convert(
    ...
    let hinting = resolve_hinting(text_node, state);
