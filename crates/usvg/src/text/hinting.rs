...
/// Whether an element is hinted at all.
///
/// Corresponds to the non-standard `-resvg-hinting` CSS property.
#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug, Default)]
pub enum TextHinting {
    /// Hint as configured, which is what the other `-resvg-hinting-*`
    /// properties then adjust.
    #[default]
    Auto,
...
/// Which of the two [`HintingTarget`] variants to use, without the settings
/// that only a smooth target carries.
#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug)]
pub enum HintingTargetKind {
    /// Selects [`HintingTarget::Mono`].
    Mono,
    /// Selects [`HintingTarget::Smooth`].
    Smooth,
}
